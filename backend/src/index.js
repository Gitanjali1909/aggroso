...
function logEvent(level, route, metadata = {}) {
  ...
  const pairs = Object.entries(metadata).map(([key, value]) => `${key}=${toLogValue(value)}`);
...
app.get("/health", (_req, res) => {
  ...
  res.json({ status: "ok", service: "codebase-qa-backend", timestamp });
});

app.post("/ask", async (req, res) => {
  const { codebase, question, repoUrl } = req.body || {};
  const questionLength = typeof question === "string" ? question.trim().length : 0;
...
app.listen(port, () => {
  logEvent("info", "SERVER", { Message: `Backend listening on http://localhost:${port}` });
});
